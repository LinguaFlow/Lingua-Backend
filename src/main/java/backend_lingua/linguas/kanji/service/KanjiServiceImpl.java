...
public class KanjiServiceImpl implements KanjiService {
    ...
    public void processKanjiData(String s3Key, Object kanjiDetails) {
        ...
        var kanji = repository.findByS3Key(s3Key)
                .orElseThrow(() -> new BusinessException(HttpResponse.FailureStatus.KANJI_TASK_NOT_FOUND));

        if (!kanjiStatusService.canProcessTask(kanji.getId())) {
            log.warn("⚠️ 처리 불가능한 상태 - Task ID: {}, S3 Key: {}", kanji.getId(), s3Key);
        ...
        try {
            ...
            log.error("❌ Kanji 작업 처리 실패 - ID: {}, 파일명: '{}', 오류: {}",
                    kanji.getId(), kanji.getBookName(), e.getMessage());
            throw e;
        }
    }
